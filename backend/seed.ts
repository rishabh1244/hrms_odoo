...
const FIRST_NAMES = ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Hank", "Ivy", "Jack",
  ...
  "Uma", "Victor", "Wendy", "Xander", "Yara", "Zane", "Aria", "Blake", "Cora", "Dylan"];
  ...
  try {
    ...
    const hr = await client.query(
      `INSERT INTO users (company_name, name, email, phone, employee_id, role, password_hash, status)
       ...
       RETURNING id`,
      ["Acme Corp", "Sarah HR", "sarah.hr@acme.com", "9876543210", "ACSAHR20260001", "hr", hrPass, "present"],
    );
    ...
    await client.query(
      ...
      [hrId, "1990-03-15", 36, "2020-01-15", "HR", "HR Manager", "123 Admin St, New York, NY"],
    );
    ...
    for (let d = 1; d <= 30; d++) {
      ...
      const status = d % 7 === 0 ? "absent" : d % 5 === 0 ? "halfday" : "present";
      const checkIn = status === "absent" ? null : `09:${rand(0, 30).toString().padStart(2, "0")}`;
      const checkOut = status === "absent" ? null : status === "halfday" ? `13:${rand(0, 30).toString().padStart(2, "0")}` : `18:${rand(0, 30).toString().padStart(2, "0")}`;
      await client.query(
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const statuses = ["present", "present", "present", "absent", "leave", "halfday", "present"];
      const currentStatus = pick(statuses);

      const emp = await client.query(
        `INSERT INTO users (company_name, name, email, phone, employee_id, role, password_hash, status)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`,
        ["Acme Corp", name, email, phone, employeeId, "employee", empPass, currentStatus],
      );
      ...
      await client.query(
        `INSERT INTO employee_details (user_id, date_of_birth, age, joining_date, department, designation, address)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [empId, dob.toISOString().split("T")[0], age, joiningDate.toISOString().split("T")[0], department, designation, `${rand(100, 999)} ${pick(["Oak", "Maple", "Pine", "Elm", "Cedar"])} St, ${pick(["New York", "San Francisco", "Chicago", "Austin", "Boston"])}, ${pick(["NY", "CA", "IL", "TX", "MA"])}`],
      );

      await client.query(
        `INSERT INTO payroll (user_id, monthly_wage, yearly_wage, travel_allowance, fixed_allowance, tax_deduction, effective_date)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [empId, monthlyWage, yearlyWage, travelAllowance, fixedAllowance, taxDeduction, "2026-01-01"],
      );
      ...
      for (let d = 1; d <= 30; d++) {
        ...
        const ci = st === "absent" ? null : `${rand(8, 10).toString().padStart(2, "0")}:${rand(0, 59).toString().padStart(2, "0")}`;
        const co = st === "absent" ? null : st === "halfday" ? `13:${rand(0, 30).toString().padStart(2, "0")}` : `${rand(17, 19).toString().padStart(2, "0")}:${rand(0, 59).toString().padStart(2, "0")}`;
        await client.query(
          `INSERT INTO attendance_records (user_id, date, check_in, check_out, status) VALUES ($1,$2,$3,$4,$5)`,
          [empId, date, ci, co, st],
        );
    ...
    console.log("Employee email pattern: <first>.<last><n>@acme.com / password: Emp@12345");
